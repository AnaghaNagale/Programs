...
typedef struct sperblock
{
	int TotalInodes;
	int FreeInode;
}SPERBLOCK,*PSUPERBLOCK;
...
void InitialiseSperBlock()
{
	...
	SPERBLOCKobj.TotalInodes=MAXINODE;
	SPERBLOCKobj.FreeInode=MAXINODE;
}
...
int CreateFile(char *name,int permission)
{
	...
	if(SPERBLOCKobj.FreeInode==0)
	    return -2;
	
	(SPERBLOCKobj.FreeInode)--;
...
void CloseAllFile()
{
	...
	while(i<50)
	{
		if(UFDTArr[i].ptrfiletable!=NULL)
		{	
	       if(UFDTArr[i].ptrfiletable->readoffset=0);
	       UFDTArr[i].ptrfiletable->writeoffset=0;
	       (UFDTArr[i].ptrfiletable->ptrinode->ReferenceCount)--;
		   break;
		}
...
void ls_file()
{
	...
	if(SPERBLOCKobj.FreeInode==MAXINODE)
	{
	...
	while(1)
	{
		...
		else if(count==2)
		{
			...
			else if(strcmp(command[0],"write")==0)
			{
				...
				if(fd == -1)
				{
					printf("Error : Incorrect parametes\n");
					continue;
			    }
		...
		else if(count == 3)
		{
			if(strcmp(command[0],"create")==0)
			{
				...
				if(ret>=0)
				    printf("File is successfully crated with file description: %d\n",ret);
				if(ret == -1)
					printf("ERROR : Incorrect parameters\n");
				if(ret == -2)
